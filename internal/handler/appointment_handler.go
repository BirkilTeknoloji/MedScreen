...
func (h *AppointmentHandler) CreateAppointment(c *gin.Context) {
	...
	if err := h.service.CreateAppointment(&appointment); err != nil {
		utils.SendErrorResponse(c, http.StatusInternalServerError, "Failed to create appointment", err)
		return
	}

	utils.SendSuccessResponse(c, http.StatusCreated, "Appointment created successfully", appointment)
}
...
func (h *AppointmentHandler) GetAppointment(c *gin.Context) {
	...
	if err != nil {
		utils.SendErrorResponse(c, http.StatusBadRequest, "Invalid appointment ID", err)
		return
	}

	appointment, err := h.service.GetAppointment(uint(id))
	if err != nil {
		utils.SendErrorResponse(c, http.StatusNotFound, "Appointment not found", err)
		return
	}

	utils.SendSuccessResponse(c, http.StatusOK, "Appointment retrieved successfully", appointment)
}
...
func (h *AppointmentHandler) GetAppointments(c *gin.Context) {
	...
	if doctorID != nil || patientID != nil || status != nil || startDate != nil || endDate != nil {
		...
		if err != nil {
			utils.SendErrorResponse(c, http.StatusInternalServerError, "Failed to retrieve appointments", err)
			return
		}

		meta := utils.CalculateMeta(page, limit, total)
		utils.SendSuccessResponseWithMeta(c, http.StatusOK, "Appointments retrieved successfully", appointments, meta)
		return
	}
...
func (h *AppointmentHandler) UpdateAppointment(c *gin.Context) {
	...
	if err := h.service.UpdateAppointment(uint(id), &appointment); err != nil {
		utils.SendErrorResponse(c, http.StatusInternalServerError, "Failed to update appointment", err)
		return
	}

	utils.SendSuccessResponse(c, http.StatusOK, "Appointment updated successfully", appointment)
}
...
func (h *AppointmentHandler) DeleteAppointment(c *gin.Context) {
	...
	if err := h.service.DeleteAppointment(uint(id)); err != nil {
		utils.SendErrorResponse(c, http.StatusInternalServerError, "Failed to delete appointment", err)
		return
	}

	utils.SendSuccessResponse(c, http.StatusOK, "Appointment deleted successfully", nil)
}
