...
func (h *AllergyHandler) CreateAllergy(c *gin.Context) {
	...
	utils.SendSuccessResponse(c, http.StatusCreated, constants.SUCCESS_ALLERGY_CREATED, "Allergy created successfully", allergy)
}
...
func (h *AllergyHandler) GetAllergy(c *gin.Context) {
	...
	if err != nil {
		utils.SendErrorResponse(c, http.StatusBadRequest, constants.ERROR_INVALID_ALLERGY_ID, "Invalid allergy ID", err)
		return
	}

	allergy, err := h.service.GetAllergy(uint(id))
	if err != nil {
		utils.SendErrorResponse(c, http.StatusNotFound, constants.ERROR_ALLERGY_NOT_FOUND, "Allergy not found", err)
		return
	}

	utils.SendSuccessResponse(c, http.StatusOK, constants.SUCCESS_ALLERGY_RETRIEVED, "Allergy retrieved successfully", allergy)
}
...
func (h *AllergyHandler) GetAllergies(c *gin.Context) {
	...
	if patientID != nil || severity != nil {
		...
		if err != nil {
			utils.SendErrorResponse(c, http.StatusInternalServerError, constants.ERROR_INTERNAL_SERVER, "Failed to retrieve allergies", err)
			return
		}

		meta := utils.CalculateMeta(page, limit, total)
		utils.SendSuccessResponseWithMeta(c, http.StatusOK, constants.SUCCESS_ALLERGIES_RETRIEVED, "Allergies retrieved successfully", allergies, meta)
		return
	}
...
func (h *AllergyHandler) UpdateAllergy(c *gin.Context) {
	...
	utils.SendSuccessResponse(c, http.StatusOK, constants.SUCCESS_ALLERGY_UPDATED, "Allergy updated successfully", allergy)
}
...
func (h *AllergyHandler) DeleteAllergy(c *gin.Context) {
	...
	utils.SendSuccessResponse(c, http.StatusOK, constants.SUCCESS_ALLERGY_DELETED, "Allergy deleted successfully", nil)
}
