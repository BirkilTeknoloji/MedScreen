...
func (h *SurgeryHistoryHandler) CreateSurgeryHistory(c *gin.Context) {
	...
	utils.SendSuccessResponse(c, http.StatusCreated, constants.SUCCESS_SURGERY_HISTORY_CREATED, "Surgery history created successfully", surgery)
}
...
func (h *SurgeryHistoryHandler) GetSurgeryHistory(c *gin.Context) {
	...
	if err != nil {
		utils.SendErrorResponse(c, http.StatusBadRequest, constants.ERROR_INVALID_SURGERY_HISTORY_ID, "Invalid surgery history ID", err)
		return
	}

	surgery, err := h.service.GetSurgeryHistory(uint(id))
	if err != nil {
		utils.SendErrorResponse(c, http.StatusNotFound, constants.ERROR_SURGERY_HISTORY_NOT_FOUND, "Surgery history not found", err)
		return
	}

	utils.SendSuccessResponse(c, http.StatusOK, constants.SUCCESS_SURGERY_HISTORY_RETRIEVED, "Surgery history retrieved successfully", surgery)
}
...
func (h *SurgeryHistoryHandler) GetSurgeryHistories(c *gin.Context) {
	...
	if patientID != nil || startDate != nil || endDate != nil {
		...
		if err != nil {
			utils.SendErrorResponse(c, http.StatusInternalServerError, constants.ERROR_INTERNAL_SERVER, "Failed to retrieve surgery histories", err)
			return
		}

		meta := utils.CalculateMeta(page, limit, total)
		utils.SendSuccessResponseWithMeta(c, http.StatusOK, constants.SUCCESS_SURGERY_HISTORIES_RETRIEVED, "Surgery histories retrieved successfully", surgeries, meta)
		return
	}
...
func (h *SurgeryHistoryHandler) UpdateSurgeryHistory(c *gin.Context) {
	...
	utils.SendSuccessResponse(c, http.StatusOK, constants.SUCCESS_SURGERY_HISTORY_UPDATED, "Surgery history updated successfully", surgery)
}
...
func (h *SurgeryHistoryHandler) DeleteSurgeryHistory(c *gin.Context) {
	...
	utils.SendSuccessResponse(c, http.StatusOK, constants.SUCCESS_SURGERY_HISTORY_DELETED, "Surgery history deleted successfully", nil)
}
